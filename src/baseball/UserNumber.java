...
public class UserNumber {
    ...
    public int findStrike(List<Integer> computerNumber) { // TODO - boolean 타입으로 하다가 생각이 바꼈음
        int strike = 0;   // TODO - 왜냐하면 결국에는 true 인지, false 인지 만 반환할 것 이라고 생각했고 다른 클래스에서 strike 랑 Ball 을 가지고 가서 비교하는게 별로 안좋다고 판단 했음
    ...
    public int findLocation(int strike, int userNumber, int computerNumber) { // 위치까지 같은지 확인
    ...
    public int findBall(List<Integer> computerNumber) {
        int ball = 0;
        for (int i = 0; i < userNumbers.size(); i++) { // 여기서는 포이치를 쓰게되면 유저숫자, 컴퓨터숫자 각각 따로 forEach 를 만들어서 값을 하나씩 가져와야 하기 때문에 그냥 for 문씀
            if (computerNumber.contains(userNumbers.get(i))) {
                ball += findLocation(ball, userNumbers.get(i), computerNumber.get(i));
            }
        }
        return ball;
    }
